...
type alias Is e =
    { e | state : Mechanics.State
        , id : EntityID
    }

type alias Entity = Is {}
...
type alias Ball = Circular Entity
...
elastic : Float -- the strength of the elastic
       -> Is thing
       -> HasField (Is thing)
elastic strength being =
    let
        field locus body =
            let
                distance_x = x locus - x_ body
                distance_y = y locus - y_ body
                accel_x = distance_x * strength
                accel_y = distance_y * strength
            in
                [ accel_x, accel_y ]
     in
        { being = being
...
friction : Float -- the coefficient of drag
        -> HasField ()
friction drag =
    let
        field _ body =
            let
                velocity_x = vx_ body
                velocity_y = vy_ body
                decel_x = decelerate drag velocity_x
                decel_y = decelerate drag velocity_y
            in
                [ decel_x, decel_y ]
     in
        { being = ()
...
actOn accels dt ({ state } as body) =
    let
        ...
        zeroAccel =
            ...
            always [0, 0]
        totalAccel =
            accels |> List.foldr addAccels zeroAccel
